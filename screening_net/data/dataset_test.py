...
class bdd_vgl_gallery(Dataset):
    ...
    def __getitem__(self, idx):
        img_name, vid_name = self.gallery_list[idx].split(' ')

        sat_image = self.get_sample(img_name, vid_name)

        return sat_image


    def get_sample(self, img_name, vid_name):

        mode = 'train'
        if not self.train:
            mode = 'val'

        image_path = os.path.join(self.root, 'satelite_data', mode, vid_name, img_name+'.jpeg')
    ...
    def get_list(self):
        ...
        gallery_list = os.path.join(self.root, 'gallery_gps_tiny.list')
