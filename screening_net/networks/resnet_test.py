...
class TestResNet(nn.Module):
    ...
    def __init__(self, name='resnet18', head='mlp', feat_dim=128, bsz=4, num_frames=32):
        super(TestResNet, self).__init__()
        # i_model_fun, i_dim_in = model_dict[name]
        #v_model_fun, v_dim_in = model_dict['vresnet18']
        v_model_fun, v_dim_in = model_dict[name]
        # self.i_encoder = i_model_fun(pretrained=False)
        self.v_encoder = v_model_fun(pretrained=False)
    ...
    def forward(self, x):
        ...
        # take average of all satelite images
        # print(v_feat.shape)
        img_features = torch.split(v_feat, self.bsz, dim=0)
        # print(img_features[0].shape)
        img_features = [f.unsqueeze(2) for f in img_features]
        # print(img_features[0].shape)
        v_feat = torch.cat(img_features, dim=2)
        # print(v_feat.shape)
        out = self.avgpool(v_feat)
        # print(out.shape)
        v_feat = torch.flatten(out, 1)
        # print(v_feat.shape)
