...
def find_top_k(preds, gt_id, gallery_ids, gt_vid, gallery_vids, opt):
    ...
    video = gt_id.split('_')[0]
    
    if video != gt:
        if len(gt_list) > 0 and gt:
            print('length of list:', len(gt_list))
                    
            link[gt] = gt_list # values for a video added to dictionary
                    
            gt_list = []       
                
                 
        gt = video # video name updated    
                               
    else:
        gt_list.append(gallery_ids[preds[0]])
        
    # link[gt] = gallery_ids[preds[0]] 
...
def evaluate(v_emb, g_emb, opt):
    ...
    #with open('./retrieved/top_pred.json','w') as fp:
    #    fp.write(json.dumps(link))   
    #fp.close()
    
    #with open('./finetuning_Laerial/top_pred.json','w') as fp:
    with open('./screening_net/retrieved/top_finetuning_pred.json','w') as fp:
        fp.write(json.dumps(link))   
    fp.close()
