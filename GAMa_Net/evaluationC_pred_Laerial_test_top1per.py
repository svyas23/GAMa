...
def get_embeddings(opt):
    ...
    base_path = os.path.split(opt.weights)[0]
    #embeddings_name = os.path.join(base_path, 'embeddings_gps_C_full.npy') # centered maps with val_gps
...
def get_ids(opt):
    ...
    #gt_ids = [x.split()[2] for x in open(gt_path, 'r').readlines()] # aerial view ids e.g. 154379_197218 #val_gps_day.list
    ...
    #gallery_ids = [(x.split( )[2]) for x in open(gallery_path, 'r').readlines()] # aerial view ids e.g. 154379_197218
    #gallery_ids = [str(1+int((x.split("/")[1]).split('.')[0])) for x in open(gallery_path, 'r').readlines()] # A # e.g. 1
    #gallery_vids = [((x.split( )[0]).split("/")[0]) for x in open(gallery_path, 'r').readlines()] # e.g. b1c66a42-6f7d68ca
    #gallery_vids = [((x.split( )[0])+'_'+x.split( )[1]) for x in open(gallery_path, 'r').readlines()] # e.g. b1c66a42-6f7d68ca/02.jpeg_1
    ...
    L2S_list = './GAMa_Net/retrieved/epoch150f/L_top1per.json'
    
    L2S_dict = json.load(open(L2S_list, 'r'))

    return gt_ids, gallery_ids, gt_vids, gallery_vids, gallery_vids1, L2S_dict
...
def find_top_k(preds1, gt_id, gallery_ids, gt_vid, gallery_vids, gallery_vids1, L2S_dict, opt):
    
    #global early_query, preds
    top_k = np.zeros(4) # [0, 0, 0, 0] # top-1, top-5, top-10, top-1%
    #top_k = np.zeros(6) # [0, 0, 0, 0, 0, 0] # top-1, top-5, top-10, top-1%, top-2%, top-100
    FLAG = True # to avoid unncessary distance cal e.g if added to top-1 automatically add to top-5 etc.
    #use preds, updated list after removing undesired from preds1 by matching video name with gallery_ids[preds[0]]
    preds = []
    vids_list = []  
    vids_list = L2S_dict[gt_id.split('_')[0]]
    #print(vids_list, 'vids_list')  
    
    #if gt_id.split('_')[0] != early_query:
    
    #    early_query = gt_id.split('_')[0]
    #    preds = []
        
    #    for i, value in enumerate(preds1):
            
    #        if gallery_ids[value].split('_')[0] in vids_list:
    #            #print(gallery_ids[value], 'gallery_ids[value]')
    #            preds.append(preds1[i]) # or index
...
def save_results(gt_vid, gt_id, preds, gallery_vids, gallery_ids, gallery_vids1, opt):
    # old: #gt_id e.g. b1c66a42-6f7d68ca_1
    # old: # preds[i] e.g. 23973
    # old: # gallery_ids[preds[i]] e.g b6f77fda-46184a8c_5
    # old: # g_id e.g. b6f77fda-46184a8c_5
    ...
    s_path = os.path.join('./predictions_2dcnn32_top1per', str(gt_vid))
    ...
    for i in range(4):
        image_name = str(f_sec*30+15+i*4).zfill(4) + '.png'
    ...
    #v_ids = [gallery_vids[i] for i in pred_ids] #gallery_vids # e.g. b1c66a42-6f7d68ca/02.jpeg_1
    #v_ids = [gallery_vids[i] for i in preds] #gallery_vids # e.g. b1c66a42-6f7d68ca/02.jpeg_1
    #print("v_ids", v_ids)
    #print("pred_ids", pred_ids)

    for i, g_id in enumerate(pred_ids):
        
        #g_id1 = g_id.split('_')[1]  # g_id e.g. bc9e1979-f1fb1692_4
        #g_id1 = "{:02d}".format(int(g_id1)) # g_id e.g. bc9e1979-f1fb1692_4
        #vid_name1 = g_id.split('_')[0] # g_id e.g. bc9e1979-f1fb1692_4
        ...
        #src_img = os.path.join(opt.data_folder, 'satelite_data', 'val_gps', vid_name, g_id1+'.jpeg')
        #print(src_img)
...
def get_scores(inds, gt_ids, gallery_ids, gt_vids, gallery_vids, gallery_vids1, L2S_dict, opt):
    ...
    for i in range(num_samples):
        ...
        c_top_k = find_top_k(preds, gt_ids[i], gallery_ids, gt_vids[i], gallery_vids, gallery_vids1, L2S_dict, opt)
        top_k += c_top_k
...
def evaluate(v_emb, g_emb, opt):
    ...
    gt_ids, gallery_ids, gt_vids, gallery_vids, gallery_vids1, L2S_dict = get_ids(opt)
    ...
    for b in range(num_batches):
        ...
        c_top_k, c_error = get_scores(inds, gt_ids[b*chunk:(b+1)*chunk], gallery_ids, gt_vids[b*chunk:(b+1)*chunk], gallery_vids, gallery_vids1, L2S_dict, opt)
    ...
    if num_batches*chunk < num_samples:
        ...
        c_top_k, c_error = get_scores(inds, gt_ids[num_batches*chunk:], gallery_ids, gt_vids[num_batches*chunk:], gallery_vids, gallery_vids1, L2S_dict, opt)
